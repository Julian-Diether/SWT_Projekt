...
std::string mypath = "playlists/"; // AUCH GEAENDERT ZUM TESTEN DER FUNKTOIONEN JIOAHFJDHFOSDJFIOSDJFIO -----------------------
...
//GEAENDERTE FUNKTION FUER UNSEREN TESTFALL!!!!!!! NORMAL FUNKTIONIERT SIE ANDERS!!!!
//!!!!!!!!!
nlohmann::json myreadfile(int select2, std::string testuserinput1) {
    ...
    mydashedline();
    //while (repeatread) {  ----------- ------testtest haben wie geaendert
        ...
        std::cout << testuserinput1 << std::endl;
        filenameread = testuserinput1;//  ----------- ------testtest haben wie geaendert
        //std::getline(std::cin, filenameread);  ----------- ------testtest haben wie geaendert
        ...
        if(!std::filesystem::exists(filenameread)) {
            ...
            //if(myjanein("Eingegebener Dateiname existiert nicht! Erneut versuchen?")) {
                ...
                //continue; ----------- ------testtest haben wie geaendert
            //} else {
            //    main_menu();
            //}
        }

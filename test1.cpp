...
void modifyTitle(std::string& title) {
    std::cout << "Current Title: " << title << std::endl;
    std::cout << title;

    char ch;
    while ((ch = getch()) != '\r') { // Loop until Enter key is pressed
        if (ch == '\b') { // Backspace
            if (!title.empty()) {
                std::cout << "\b \b"; // Clear the last character on the terminal
                title.pop_back(); // Remove the last character from the title
            }
        } else {
            std::cout << ch;
            title += ch; // Append the character to the title
        }
    }
    std::cout << std::endl;
...
int main() {
    std::string title = "My Song Title";

    // Modify the title
    modifyTitle(title);

    // Retrieve the modified title and store it in the file
    std::cout << "Modified Title: " << title << std::endl;
